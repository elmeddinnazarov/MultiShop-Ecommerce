window.addEventListener('load', () => {
    let state = window.location.search.slice(1,).split('&').map(e => e.split('=')).filter(e => e[0])

    const filterButton = document.querySelector('.filter-button')

    const colors = state.filter(clr => clr[0] === 'color')
    ...
    if (colors.length) {
        colors.forEach(clr => {
            const colorInput = document.querySelector('#color-'+clr[1]);
    ...
    colorInputs.forEach(i => {
        i.addEventListener('change', (e) => {
            ...
            const title = e.target.getAttribute('title-data')
            if (checked) {
                allColorInput.checked = false;
    ...
    const sizes = state.filter(clr => clr[0] === 'size')
    ...
    if (sizes.length) {
        sizes.forEach(clr => {
            const sizeInput = document.querySelector('#size-'+clr[1]);
    ...
    minPriceEl.oninput = e => {
        ...
        if (value >= minLimit && value < maxValue) {
            ...
            priceError = true
            filterButton.style.opacity = '0.1'
